...
class MoviesDetailsCubit extends Cubit<MoviesDetailsState> {
  ...
  Future<dynamic> getMoveisDetails({required int id})async{
    try{
      ...
      final Response response =await api.get(Endpoint.baseUrlMoveis, Endpoint.moveisDetails,queryParameters: {
      'movie_id' : id, 
    });
    if(response.statusCode == 200 || response.statusCode == 201){
      if(response.data is String){
        MovisDetails movisDetails = MovisDetails.fromJson(jsonDecode(response.data));
        emit(MoviesDetailSucsess(moveisList: movisDetails.data!.movie!));
        ...
        emit(MoviesDetailFaliure(errorMessage: response.statusMessage ?? 'UnKownError'));
      }
    }else{
      emit(MoviesDetailFaliure(errorMessage: response.statusMessage ?? 'UnKownError'));
      
      print('===== Error Response: ${response.statusMessage ?? "Unknown error"}');
    }
